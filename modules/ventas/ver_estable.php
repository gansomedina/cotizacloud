...
$url_vta      = url_publica('v/' . $venta['slug']);
...
@media(max-width:900px){.detail-layout{flex-direction:column}.col-side{width:100%;position:static}}
...
/* ── ARTÍCULOS — grid 4 columnas: descripcion | cant | p.unit | total ── */
...
.line-edit-btn,.line-del-btn{background:none;border:none;cursor:pointer;font-size:11px;opacity:.5;padding:0 2px;transition:opacity .1s}
...
.item-sku{font:400 11px var(--num);color:var(--t3);margin-top:1px}
.item-desc{font:400 15px var(--body);color:var(--t3);margin-top:3px;line-height:1.4;white-space:pre-line}
.item-cell{text-align:right;font:400 13px var(--num);color:var(--t2);padding-top:2px}
.item-total{text-align:right;font:600 13px var(--num);padding-top:2px}
.item-add-btn{width:100%;padding:11px;border:none;background:var(--bg);font:600 13px var(--body);color:var(--t2);cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;transition:all .12s;border-top:1px solid var(--border)}
...
function renderLineas(){
  ...
  el.innerHTML = lineas.map((l,i) => `
    <div class="item-row">
      <div>
        <div class="item-name">${escHtml(l.titulo)}
        ...
        ${l.sku ? `<div class="item-sku">${escHtml(l.sku)}</div>` : ''}
        ${l.descripcion ? `<div class="item-desc">${escHtml(l.descripcion).replace(/\n/g,'<br>')}</div>` : ''}
      </div>
      <div class="item-cell">${fmtCant(l.cantidad)}</div>
      <div class="item-cell">${l.precio_unit > 0 ? fmtMoney(l.precio_unit) : '—'}</div>
      <div class="item-total">${fmtMoney(l.subtotal)}</div>
    </div>`).join('');
}
...
// ── Sheets ──
function openSheet(id){document.getElementById('ov-'+id).classList.add('open');document.getElementById(id).classList.add('open')}
function closeSheet(id){document.getElementById('ov-'+id).classList.remove('open');document.getElementById(id).classList.remove('open')}
