...
$asesores = DB::query(
    "SELECT u.id, u.nombre, u.empresa_id,
            ...
            us.bonus_cierre, us.ticket_promedio,
            us.mesa_pedidas, us.mesa_atendidas, us.s_mesa,
            us.mesa_dias_vencidos, us.castigo_seguimiento
     FROM usuarios u
    ...
    <?php foreach ($rt_todas as $rt_g): ?>
      ...
      <div style="display:flex;align-items:flex-start;gap:8px;padding:9px 12px;border-bottom:1px solid var(--border)">
        ...
        <div style="flex:1;min-width:0">
          <div style="display:flex;align-items:center;gap:6px"><span style="font:600 13px 'Inter',sans-serif;color:var(--text)"><?= e($rt_f['nombre']) ?></span><span style="font:750 13px 'Inter',sans-serif;color:<?= $rt_c ?>"><?= (int)$rt_f['score'] ?><span style="font-size:9px;color:var(--t3)">/100</span></span><?php if (!empty($rt_f['flag'])): ?><span style="font-size:11px">🚩</span><?php endif; ?></div>
          <div style="font:500 11px 'Inter',sans-serif;color:var(--t3);margin-top:1px">cerró <b style="color:var(--text)"><?= (int)$rt_f['cierres7'] ?></b>/<?= (int)$rt_f['trabajo7'] ?> trab<?php if ($rt_f['desc7'] > 0): ?> · 🗑<?= (int)$rt_f['desc7'] ?><?php $rt_d = []; if ($rt_f['sincita7'] > 0) $rt_d[] = (int)$rt_f['sincita7'].' s/cita'; if ($rt_f['rapido7'] > 0) $rt_d[] = (int)$rt_f['rapido7'].' ráp'; if ($rt_d): ?> <span style="color:var(--r)">(<?= implode(', ', $rt_d) ?>)</span><?php endif; ?><?php endif; ?><?php if ($rt_f['nocontesta7'] > 0): ?> · <span style="color:var(--r)">n/c <?= (int)$rt_f['nocontesta7'] ?></span><?php endif; ?><?php if (!empty($rt_f['venc_sube'])): ?> · <span style="color:var(--a);font-weight:700">⏰▲</span><?php endif; ?><?php if (!empty($rt_f['citas_baja'])): ?> · <span style="color:var(--a);font-weight:700">📅↓</span><?php endif; ?></div>
