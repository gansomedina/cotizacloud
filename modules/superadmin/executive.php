...
<!-- ══ FEEDBACK DE CLIENTES (todas mis empresas) ═════════════ -->
<?php
// Solo empresas que tengan feedback activo
$emp_fb_activas_rows = DB::query(
    "SELECT id, slug, nombre, feedback_activo FROM empresas
     WHERE id IN ({$emp_ids}) AND feedback_activo = 1"
);
$emp_fb_ids = array_column($emp_fb_activas_rows, 'id');

if (!empty($emp_fb_ids)):
    $fb_ids_csv = implode(',', array_map('intval', $emp_fb_ids));
    $fb_recientes = DB::query(
        "SELECT f.stars, f.comentario, f.created_at, f.empresa_id,
         ...
         LEFT JOIN clientes cl ON cl.id = c.cliente_id
         WHERE f.empresa_id IN ({$fb_ids_csv})
    ...
    $fb_resumen_emp = DB::query(
        "SELECT empresa_id, COUNT(*) AS total,
                ...
                SUM(CASE WHEN stars=5 THEN 1 ELSE 0 END) AS cinco,
                SUM(CASE WHEN stars<=2 THEN 1 ELSE 0 END) AS bajos
         FROM cot_feedbacks
         WHERE empresa_id IN ({$fb_ids_csv})
         GROUP BY empresa_id"
    );
    $fb_idx = []; foreach ($fb_resumen_emp as $r) $fb_idx[(int)$r['empresa_id']] = $r;
?>
<style>
.fbexec-stars{color:#f59e0b;letter-spacing:1px;font-size:13px}
.fbexec-stars-empty{color:#374151;letter-spacing:1px;font-size:13px}
.fbexec-card{padding:12px 14px;border-radius:8px;margin-bottom:8px;background:rgba(255,255,255,.03);border-left:3px solid var(--bd)}
...
.fbexec-meta{display:flex;justify-content:space-between;gap:8px;font:500 11px 'Inter',sans-serif;color:var(--t3);margin-bottom:4px}
.fbexec-text{font:400 12px 'Inter',sans-serif;color:var(--t2);line-height:1.4;margin-top:4px;font-style:italic}
.fbexec-cot{font:400 10px 'Inter',sans-serif;color:var(--t3);margin-top:4px;font-family:var(--num)}
.fbexec-resumen{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:14px}
.fbexec-emp-card{flex:1;min-width:120px;background:rgba(255,255,255,.04);padding:8px 12px;border-radius:6px}
.fbexec-emp-prom{font:700 18px var(--num);color:#f59e0b}
.fbexec-emp-tot{font:500 10px 'Inter',sans-serif;color:var(--t3);margin-top:2px}
</style>
<div class="sec" style="margin-top:28px">
    ...
    <div class="tbl-card" style="padding:20px;text-align:center;color:var(--t3);font:500 13px 'Inter',sans-serif">
        Sin feedback aún en tus empresas
    </div>
    <?php else: ?>
    <div class="tbl-card" style="padding:14px 16px">
        ...
        <?php foreach ($emp_fb_activas_rows as $emp):
            ...
            $bajos = $r ? (int)$r['bajos'] : 0;
        ?>
        <div class="fbexec-emp-card">
            <div style="display:flex;align-items:center;gap:6px;margin-bottom:4px">
                <span class="tag" style="background:<?= $ec['color'] ?>;font-size:8px;min-width:28px;padding:3px 6px"><?= $ec['short'] ?></span>
                <?php if ($total > 0): ?>
                    <span class="fbexec-emp-prom"><?= number_format($prom, 2) ?></span>
                <?php else: ?>
                    <span style="color:var(--t3);font:500 12px 'Inter',sans-serif">Sin feedback</span>
                <?php endif; ?>
            </div>
            <?php if ($total > 0): ?>
                <div class="fbexec-emp-tot">
                    <?= $total ?> calificaci<?= $total === 1 ? 'ón' : 'ones' ?>
                    <?php if ($bajos > 0): ?>· <span style="color:#ef4444"><?= $bajos ?> ≤2★</span><?php endif; ?>
                </div>
            <?php endif; ?>
        </div>
        <?php endforeach; ?>
        ...
        <div style="max-height:380px;overflow-y:auto">
            ...
            <div class="fbexec-card s<?= $stars ?>">
                <div class="fbexec-meta">
                    <div style="display:flex;align-items:center;gap:6px">
                        <span class="tag" style="background:<?= $ec['color'] ?>;font-size:8px;min-width:24px;padding:2px 5px"><?= $ec['short'] ?></span>
                        <span class="fbexec-stars"><?php for($i=0;$i<$stars;$i++) echo '★'; ?></span><span class="fbexec-stars-empty"><?php for($i=$stars;$i<5;$i++) echo '★'; ?></span>
                        <span style="color:var(--t2);font-weight:600"><?= e($f['asesor'] ?? '—') ?></span>
                    </div>
                ...
                <div class="fbexec-cot">
                    ...
                    <?php if (!empty($f['cot_titulo'])): ?> · <?= e(mb_substr($f['cot_titulo'],0,50)) ?><?php endif; ?>
                </div>
            </div>
            <?php endforeach; ?>
        </div>
