...
class ActividadScore
{
    ...
    public static function diagnostico(array $s, ?array $ctx = null): string
    {
        ...
        $max_frases = match(true) { $score >= 80 => 4, $score >= 70 => 5, $score >= 55 => 6, $score >= 40 => 7, default => 5 };
        ...
        if ($h_down > $h_up && $h_down > 2) {
            if ($score >= 70) {
                ...
                $v = ["$h_down clientes bajando de actividad, $h_up regresaron. La cartera se está enfriando.", "Más clientes perdiendo interés que regresando. Revisa el Radar."];
            }
            $frases[] = $v[$rot % count($v)];
        ...
        // ═══ 6. HISTÓRICO ═══
        if ($volumen_bajo && $score < 75) $frases[] = "Estás por debajo del promedio mensual de ventas de la empresa.";
        ...
        if ($score >= 80) {
            ...
            $v = [" 💡 Los números necesitan mejorar. Entra al Radar, enfócate en lo que tiene actividad y cotiza más.", " 💡 Revisa el Radar, enfócate en cerrar lo que tiene movimiento y sigue cotizando. Los números tienen que subir.", " 💡 El Radar te muestra dónde hay oportunidad real. Enfócate ahí, cotiza más y trabaja en mejorar el cierre."];
        }
        $frases[] = $v[$rot];
