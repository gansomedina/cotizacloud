...
$venta = DB::row(
    "SELECT v.*,
            ...
            e.sitio_web AS emp_web, e.moneda, e.terminos,
...
<div class="empresa-header">
  <div class="empresa-inner">
    ...
    <div class="empresa-contacto">
      ...
      <?php endif; ?>
      <?php if ($venta['emp_tel']): ?>
      <a href="https://wa.me/52<?= preg_replace('/\D/','',$venta['emp_tel']) ?>">💬 WhatsApp</a>
      <?php endif; ?>
...
<div class="main">
  ...
  <div class="card">
    <div class="venta-hdr">
      ...
      <div class="venta-grid">
        ...
        <?php if ($venta['asesor_nombre']): ?>
        <div><div class="meta-lbl">Asesor</div><div class="meta-val"><?= e($venta['asesor_nombre']) ?></div></div>
        <?php endif; ?>
  ...
  <div class="card">
    ...
    <div class="item-row">
      <div class="item-body">
        ...
        <?php if ($l['descripcion']): ?><div class="item-desc"><?= e($l['descripcion']) ?></div><?php endif; ?>
      </div>
      <div class="item-r">
        <div class="item-qty"><?= $l['precio_unit'] > 0 ? number_format($l['cantidad'],2).' × '.fmt_v($l['precio_unit']) : '—' ?></div>
        <div class="item-amt"><?= $l['precio_unit'] > 0 ? fmt_v($l['subtotal']) : fmt_v(0) ?></div>
      </div>
    </div>
    <?php endforeach; ?>
  </div>
  ...
  <div class="card">
    <?php
    $subtotal_v = $venta['subtotal'] ?? array_sum(array_column($lineas,'subtotal'));
    ?>
    ...
    <div class="tot-row disc"><span class="tot-lbl">Descuento especial</span><span class="tot-val">-<?= fmt_v($venta['descuento_auto_amt']) ?></span></div>
    <?php endif; ?>
    <?php if ((float)($venta['cupon_monto'] ?? 0) > 0): ?>
    <div class="tot-row disc"><span class="tot-lbl">Cupón <?= e($venta['cupon_codigo'] ?? '') ?></span><span class="tot-val">-<?= fmt_v($venta['cupon_monto']) ?></span></div>
    <?php endif; ?>
  ...
  <div class="card">
    <?php foreach ($recibos as $r):
        ...
        $rec_data = json_encode([
            ...
            'monto'     => fmt_v(abs($r['monto'])),
            'forma'     => ucfirst($forma) . ($r['referencia'] ? ' · '.$r['referencia'] : ''),
            'ref'       => $r['referencia'] ?? '—',
            'fecha'     => date('d M Y, g:i A', strtotime($r['created_at'])),
            'prev'      => fmt_v($r['pagado_antes'] ?? 0),
            'saldo'     => fmt_v($r['saldo_despues'] ?? $saldo),
            'cancelacion' => $es_cancelacion,
        ]);
        // Recibo de cancelación (vinculado)
        $rec_cancel_data = null;
        if ($es_cancelado && $r['cancelado_por_id']) {
            $rc = DB::row("SELECT numero, created_at, monto FROM recibos WHERE id=?", [$r['cancelado_por_id']]);
            if ($rc) {
                $rec_cancel_data = json_encode([
                    'folio'   => $rc['numero'],
                    'concepto'=> 'Cancelación de '.$r['numero'],
                    'monto'   => fmt_v(abs($rc['monto'])),
                    'forma'   => '—', 'ref' => '—',
                    'fecha'   => date('d M Y, g:i A', strtotime($rc['created_at'])),
                    'prev'    => '—', 'saldo' => '—',
                    'cancelacion' => true,
                ]);
            }
    ...
    <div class="abo-row <?= ($es_cancelado || $es_cancelacion) ? 'abo-cancelado' : '' ?>">
      ...
      <div class="abo-body">
        ...
        <div class="abo-forma"><?= ucfirst($forma) ?><?= $r['referencia'] ? ' · '.$r['referencia'] : '' ?></div>
        <div class="abo-btns">
  ...
  <!-- BOTONES -->
  <?php if ($venta['emp_tel']): ?>
  <a href="https://wa.me/52<?= preg_replace('/\D/','',$venta['emp_tel']) ?>"
...
<div class="fac">
  ...
  <div class="fac-info-row">
    ...
    <?php if ($venta['asesor_nombre']): ?><div class="fac-info-cell"><div class="fac-info-lbl">Asesor</div><div class="fac-info-val"><?= e($venta['asesor_nombre']) ?></div></div><?php endif; ?>
  ...
  <table class="fac-tbl">
    ...
    <tr>
      ...
      <td>
        ...
        <?php if ($l['descripcion']): ?><div class="td-desc"><?= e(mb_substr($l['descripcion'],0,120)) ?></div><?php endif; ?>
      </td>
  ...
  <div class="fac-bottom">
    <div class="fac-pagos">
      ...
      <div class="fac-pago-row <?= $es_c?'fac-pago-cancelado':'' ?>">
        <div>
          ...
          <div class="fac-pago-sub"><?= date('d M Y', strtotime($r['created_at'])) ?> · <?= ucfirst($r['forma_pago'] ?? '') ?><?= $r['referencia']?' · '.e($r['referencia']):'' ?></div>
        </div>
    ...
    <div class="fac-totales">
      ...
      <?php if ((float)($venta['descuento_auto_amt'] ?? 0) > 0): ?>
      <div class="fac-tot-row"><span class="fac-tot-lbl">Descuento</span><span class="fac-tot-val">-<?= fmt_v($venta['descuento_auto_amt']) ?></span></div>
      <?php endif; ?>
