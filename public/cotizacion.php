...
// ── Determinar dominio correcto de cookie según host actual ──
// .cotiza.cloud para subdominios. Vacío (host-only) para dominios custom.
// Replicar lógica de api/safari_bridge.php para no romper cookies en custom domain.
...
if ($visitor_id_cookie === '') {
    ...
    setcookie('cz_vid', $visitor_id_cookie, [
        'expires'  => time() + 730 * 86400,
        'path'     => '/',
        'domain'   => $cookie_domain,
        'secure'   => !DEBUG,
...
if (!es_bot($ua) && in_array($cot['estado'], ['enviada','vista','aceptada','rechazada'])) {
    try {
        ...
        // ── CAPA 2 (IP interna) eliminada ─────────────────────────────
        // Las IPs de carrier rotan: una IP que fue del asesor pasa a un
        // cliente real → se descartaba su visita. El asesor se detecta por
        // Capa 0 (sesión) y Capa 1 (cz_vid), no por IP.
...
.qh-top{padding:18px 22px 14px;border-bottom:1px solid var(--bd)}
...
.qh-pills{grid-template-columns:repeat(6,1fr)}
.pill-m3{grid-column:span 3}
.pill-m2{grid-column:span 2}
}
...
<div class="print-info">
  ...
  <div class="qh">
    ...
    <div class="qh-pills">
      ...
      ?>
      <div class="pill pill-m2" <?php if ($vts && $vd < 0): ?>style="background:#fff5f5;border-color:#fca5a5"<?php elseif ($vts && $vd <= 3): ?>style="background:#fffbeb;border-color:#fcd34d"<?php endif; ?>>
  ...
  <table class="tbl">
    ...
    <tr>
      <td>
        ...
        <?php if ($l['descripcion']): ?><div class="idesc"><?= nl2br(str_contains($l['descripcion'], '<') ? e_html($l['descripcion']) : e_links($l['descripcion'])) ?></div><?php endif; ?>
      </td>
      <?php if (!$ocultar_cp): ?>
  ...
  <div class="items-mob">
    ...
    <div class="im">
      ...
      <?php if ($l['descripcion']): ?><div class="im-desc"><?= nl2br(str_contains($l['descripcion'], '<') ? e_html($l['descripcion']) : e_links($l['descripcion'])) ?></div><?php endif; ?>
  ...
  <table class="tbl">
    ...
    <tr>
      <td>
        ...
        <?php if ($le['descripcion']): ?><div class="idesc"><?= nl2br(str_contains($le['descripcion'], '<') ? e_html($le['descripcion']) : e_links($le['descripcion'])) ?></div><?php endif; ?>
      </td>
  ...
  <div class="items-mob">
    ...
    <div class="im">
      ...
      <?php if ($le['descripcion']): ?><div class="im-desc"><?= nl2br(str_contains($le['descripcion'], '<') ? e_html($le['descripcion']) : e_links($le['descripcion'])) ?></div><?php endif; ?>
...
.fb-star{font-size:44px;color:var(--glt);cursor:pointer;line-height:1;transition:transform .15s ease,color .15s ease,text-shadow .15s ease;user-select:none;background:none;border:0;padding:0;-webkit-tap-highlight-color:transparent;-webkit-text-stroke:1.5px var(--g);paint-order:stroke fill;text-shadow:0 1px 3px rgba(0,0,0,.08)}
.fb-star:hover{transform:scale(1.15);color:var(--gbd);text-shadow:0 2px 8px rgba(0,0,0,.12)}
