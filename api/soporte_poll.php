...
function soporte_estado_horario(): array {
    ...
    return ['online' => $online, 'msg' => $online ? ($cfg['msg_online'] ?? 'En línea') : ($cfg['msg_fuera'] ?? 'Fuera de horario'), 'saludo' => $cfg['saludo'] ?? ''];
}

// ── Lado VISITANTE ANÓNIMO (landing) por token ──
if ($rol === '') {
    $token = substr(preg_replace('/[^a-zA-Z0-9\-]/', '', (string)($_GET['token'] ?? '')), 0, 64);
    $conv  = $token !== '' ? DB::row("SELECT id, estado, no_leidos_usuario FROM soporte_conversaciones WHERE visitor_token=? AND origen='landing' ORDER BY id DESC LIMIT 1", [$token]) : null;
    $mensajes = []; $conv_id = 0; $no_leidos = 0; $estado = 'cerrada';
    if ($conv) {
        $conv_id = (int)$conv['id']; $no_leidos = (int)$conv['no_leidos_usuario']; $estado = $conv['estado'];
        foreach (DB::query("SELECT id, autor, cuerpo, created_at FROM soporte_mensajes WHERE conversacion_id=? AND id>? ORDER BY id ASC LIMIT 50", [$conv_id, $since]) as $r) {
            $mensajes[] = ['id'=>(int)$r['id'], 'autor'=>$r['autor'], 'cuerpo'=>$r['cuerpo'], 'hora'=>date('H:i', strtotime($r['created_at']))];
        }
    }
    echo json_encode(['ok'=>true, 'conversacion_id'=>$conv_id, 'estado'=>$estado, 'no_leidos'=>$no_leidos, 'horario'=>soporte_estado_horario(), 'mensajes'=>$mensajes]);
    exit;
}
